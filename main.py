...
for a in activities:
    ...
    if activity.description != "" or daysSince > 5:
        break

    response = ollama.chat(model='llama3.1', messages=[
        {
            'role': 'user',
            'content': 'Provide only an original quote that makes you think/reflect about yourself, the world, or life in '
                       'general. Do'
                       'not provide any context or explanation. Do not give any author or source. Do not surround '
                       'with quotations.'
        },
    ])
    print(response["message"]["content"])
    client.update_activity(activity.id, description=response["message"]["content"] + "\n\n-llama3.1   \n" + datetime.now()
